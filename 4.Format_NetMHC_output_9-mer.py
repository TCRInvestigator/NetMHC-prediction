...
def parse_fasta_file(fasta_file_path):
    ...
    for record in SeqIO.parse(fasta_file_path, "fasta"):
        ...
        if len(parts) > 4:
            ...
            achange = ""
        # For status, check if the header contains 'mut' or 'wt'; 
        # if not, default to 'mut' (since indel-derived FASTA files lack this info)
...
# -----------------------------------------------------
# Modified merge function to remove duplicate NetMHC rows
# -----------------------------------------------------
def merge_data(netmhc_data, fasta_data):
    # Remove duplicates from netmhc_data based on the "Peptide" column,
    # keeping only the first occurrence.
    netmhc_unique = netmhc_data.drop_duplicates(subset=["Peptide"], keep="first")
    return pd.merge(fasta_data, netmhc_unique, left_on='Epi9mer', right_on='Peptide', how='left')
...
def main(netmhc_file_path, fasta_file_path, output_file_path):
    ...
    final_data = merged_data  # No duplicate removal here
